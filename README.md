...
inputs: # Array of inputs
  - path: /tmp/php-error.log  # Path to the log named pipe
    multiline:
      enabled: true # set to 'true' if your logs are multilined
      first_line: ^\[\d{2}-\w{3}-\d{4}\s+\d{2}:\d{2}:\d{2}\] # set regexp to detect begining of multiline
      flush_interval: 2 # flush interval for multiline logs. Value in seconds.
    parsers: # Array of log parsers for extracting fields from log.
      - ^\[(?P<time>\d{2}-\w{3}-\d{4}\s+\d{2}:\d{2}:\d{2}\s\w+\/\w+)\] PHP (?P<severity>[^:]+):(\s+)?(?P<msg>.*)
    tags: # Array of additional tags which will be added to each log message
      - name: hostname    #Name of the field
        value: $HOSTNAME  #Value of the field. Add '$' prefix to get value from ENV.
      - name: event_type
        value: php.error
      - name: team
        value: $TEAM
